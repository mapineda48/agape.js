...
/** Special permission value for public endpoints */
...
/**
 * Extracts @permission and @public tags from a TypeScript source file.
 */
function extractPermissions(sourceCode: string): PermissionEntry[] {
  ...
  function visit(node: ts.Node) {
    ...
    // Extract @public or @permission tag if we found a function
    if (functionName) {
      const permission = getAccessTag(targetNode);
      if (permission) {
    ...
    if (
      ...
      const permission = getAccessTag(node);
      if (permission) {
...
/**
 * Extracts @public or @permission tag value from a node's JSDoc comments.
 * @public takes precedence and returns the special PUBLIC_PERMISSION value.
 * Sanitizes the result to remove newlines and extra whitespace.
 */
function getAccessTag(node: ts.Node): string | null {
