...
export class CacheManager {
  ...
  public static get(): CacheManager {
    if (!this.instance) {
      throw new Error("CacheManager not initialized. Call CacheManager.init(url) first.");
    }
    return this.instance;
  }
  ...
  public async getBuffer(key: string): Promise<Buffer | null> {
    const buf = await this.client.withCommandOptions(rpcCommandOptions).get(key) as Buffer | null;
    return buf ?? null;
  }

  public async setBuffer(key: string, value: Buffer, ttlSeconds?: number): Promise<void> {
  ...
  public cache<T, A extends unknown[]>(
    ...
    return async (...args: A) => {
      const key = keyBuilder ? keyBuilder(args) : defaultKeyFromArgs(baseKey, args);
