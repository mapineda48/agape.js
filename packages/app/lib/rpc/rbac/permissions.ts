...
async function parsePermissionsFromFile(
  ...
  function visit(node: import("typescript").Node) {
    ...
    let targetNode: import("typescript").Node = node;
    ...
    if (functionName) {
      ...
      const publicTag = tags.find((t: import("typescript").JSDocTag) => t.tagName?.text === "public");
      if (publicTag) {
        permissions.set(functionName, PermissionLevel.PUBLIC);
      } else {
        // Check for @permission tag
        const permTag = tags.find((t: import("typescript").JSDocTag) => t.tagName?.text === "permission");
          ...
          const permission =
            typeof permTag.comment === "string"
              ...
              : Array.isArray(permTag.comment)
                ? permTag.comment
                    .map((p: import("typescript").JSDocComment) => (typeof p === "string" ? p : p.text))
                    .join("")
                    .trim()
                : null;
...
// Uses the same logic as bin/env.ts for consistency
...
export async function initPermissions(): Promise<void> {
  if (isDev) {
    ...
    console.log(
      `🔐 Permission system initialized (${count} protected endpoints)`,
    );
  }
}
