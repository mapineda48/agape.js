...
export interface FormActions {
  ...
  pushAtPath: (path: Path, value: unknown) => void;
...
export function getByPath<T = unknown>(
  ...
  for (let i = 0; i < path.length; i++) {
    const key = path[i];
    if (curr == null || typeof curr !== "object" || !(key in (curr as Record<string | number, unknown>))) return fallback as T;
 ...
 */
function setByPath(target: Record<string | number, unknown>, path: Path, value: unknown): void {
  let curr: Record<string | number, unknown> = target;
 ...
 */
function deleteByPathAndCleanup(root: unknown, path: Path): boolean {
  ...
  if (isLastKey) {
    ...
    const shouldCleanup = deleteByPathAndCleanup(obj[key], path.slice(1));
    if (shouldCleanup) {
  ...
  return createStore<FormStore>((set) => ({
    ...
    setAtPath: (path: Path, value: unknown) => {
      ...
      set((state) => {
        ...
        const newData = { ...(state.data as Record<string | number, unknown>) };
    ...
    pushAtPath: (path: Path, value: unknown) => {
      ...
      set((state) => {
        ...
        let arr = getByPath<unknown[]>(newData, path, undefined);
    ...
    removeAtPath: (path: Path, indices: number[]) => {
      set((state) => {
        ...
        const arr =
          path.length === 0
            ? newData
            : getByPath<unknown[]>(newData, path, []);
