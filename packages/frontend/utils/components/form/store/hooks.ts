...
export function useAppDispatch() {
  ...
  return useMemo(() => {
    const dispatch = (action: Record<string, unknown>) => {
      const payload = (action.payload ?? action) as Record<string, unknown>;

      // Handle legacy action format
      if (action.type === "dict/setAtPath" || action.path !== undefined) {
        actions.setAtPath(payload.path as Path, payload.value);
        return;
      }

      if (action.type === "dict/pushAtPath") {
        actions.pushAtPath(payload.path as Path, payload.value);
        return;
      }

      if (action.type === "dict/removeAtPath") {
        actions.removeAtPath(payload.path as Path, payload.index as number[]);
        return;
      }

      if (action.type === "dict/deleteAtPath") {
        actions.deleteAtPath(payload.path as Path);
        return;
      }

      if (action.type === "dict/resetState") {
        actions.resetState(payload.state);
        return;
      }
 ...
 */
export function useAppSelector<T>(selector: (state: { form: { data: unknown } }) => T): T {
