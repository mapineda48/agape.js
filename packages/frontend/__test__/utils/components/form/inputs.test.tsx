...
describe("Extended Inputs", () => {
  ...
  describe("Text Input - Basic functionality", () => {
    ...
    it("should handle empty initial state with default value", () => {
      render(
        <Form.Root state={{}}>
          <Input.Text path="city" defaultValue="New York" data-testid="input" />
        </Form.Root>,
      );
  ...
  describe("DateTime Input - additional cases", () => {
    it("should handle materialize=false", () => {
      ...
      let _capturedState: any;

      const StateSpy = () => {
        // eslint-disable-next-line react-hooks/globals
        _capturedState = useAppSelector((state) => state.form.data);
        return null;
      };
  ...
  describe("File Input - additional cases", () => {
    ...
    it("should accumulate files when multiple is enabled", async () => {
      ...
      // Add first file
      const file1 = new File(["content1"], "photo1.jpg", {
        type: "image/jpeg",
      });
      ...
      // Add second file
      const file2 = new File(["content2"], "photo2.jpg", {
        type: "image/jpeg",
      });
