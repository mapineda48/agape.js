...
export default class AzureBlobStorage {
   ...
   */
  static reset(): void {
    this.hostCdn = undefined as unknown as string;
    this.containerClient = undefined as unknown as ContainerClient;
  }

  static async uploadFile(dir: string, file: File): Promise<string> {
    const stream: Stream.Readable = file.stream() as unknown as Stream.Readable;
