...
const imports = Object.fromEntries(Object.entries(compilerOptions.paths).map(([alias, [pattern]]) => [alias, pattern]));
 ...
 * Source Maps
 */
for await (const map of glob("**/*.map", { cwd: path.resolve("dist/web/www") })) { 
...
fs.outputJSONSync("dist/package.json", {
    ...
    imports: Object.fromEntries(
        Object.entries(imports).map(normaliceImport)
    )
}, { spaces: 2 })