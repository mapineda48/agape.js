...
import {
  listEmployees,
  type ListEmployeesParams as GetEmployeesParams,
  type EmployeeListItem as GetEmployee,
  type ListEmployeesResult as GetEmployeesResult,
...
interface Props extends GetEmployeesResult { }
...
export default function EmployeesPage(props: Props) {
  const notify = useNotificacion();
  const { navigate } = useRouter();

  const [{ filters, totalCount, employees, fetch }, setState] =
  ...
  const updateFilter = (newFilters: Partial<GetEmployeesParams>) => {
    setState({
      employees,
      totalCount,
      fetch: true,
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Gestiona tus empleados y su información
              </p>
            </div>
            <button
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {employees.map((employee) => (
                      <EmployeeRow
                        key={employee.id}
                        employee={employee}
                        onEdit={() => navigate(`../employee/${employee.id}`)}
                      />
                    ))}
                  </tbody>
                </table>
...
function EmployeeRow({
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <span
          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${employee.isActive
              ? "bg-green-100 text-green-800"
              : "bg-gray-100 text-gray-800"
            }`}
        >
          <span
            className={`h-2 w-2 rounded-full mr-2 ${employee.isActive ? "bg-green-400" : "bg-gray-400"
              }`}
          />
